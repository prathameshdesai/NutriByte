...
public class NutriProfiler {
	...
	static void createNutriProfile(Person person) {
		...
		//Adding the Energy Object first
		person.recommendedNutrientsList.add(new RecommendedNutrient(ENERGY_NUTRIENT_CODE, person.calculateEnergyRequirement()));

		float[] nutrientRequirement = person.calculateNutriRequirement();

		//Adding all the Other Nutrient Objects to the list
		for(NutriProfiler.NutriEnum nutri : NutriProfiler.NutriEnum.values())
		{
			person.recommendedNutrientsList.add(new RecommendedNutrient(nutri.getNutrientCode(), nutrientRequirement[nutri.nutriIndex]));
		}
